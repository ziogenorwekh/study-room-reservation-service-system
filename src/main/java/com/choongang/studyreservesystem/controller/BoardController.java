...
public class BoardController {
	...
	public String createPost(@ModelAttribute CreatePostDto createPostDto, Authentication authentication) {
	...
	public String boardList(@RequestParam(defaultValue = "0", name = "page") int page,
			@RequestParam(required = false, name = "keyword") String keyword,
			@RequestParam(defaultValue = "all", name = "searchType") String searchType, Model model) {
		...
		if (keyword != null && !keyword.trim().isEmpty()) {
			SearchPostDto searchDto = SearchPostDto.builder().keyword(keyword).searchType(searchType).build();
	...
	public String deleteBoard(@PathVariable Long postId, @AuthenticationPrincipal UserDetails userDetails) {
	...
	public String editForm(@PathVariable Long id, Model model, @AuthenticationPrincipal CustomUserDetails me) {
		Board board = postService.getPostByPostId(id); // ✅ 여기서도 인스턴스 호출

		boolean owner = board.getUsername().equals(me.getUsername());
		boolean admin = me.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN"));
		if (!(owner || admin)) {
	...
	public String editBoard(@PathVariable Long id, @Valid @ModelAttribute("form") UpdatePostDto updatePostDto,
			BindingResult br, @AuthenticationPrincipal CustomUserDetails user, RedirectAttributes ra, Model model) {
		// 폼 검증 실패 → 수정 폼으로
		if (br.hasErrors()) {
